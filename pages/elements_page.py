...
class WebTablePage(BasePage):
	...
	def check_search_person(self):
		delete_button = self.element_is_presents(self.locators.DELETE_BUTTON)
...
class LinksPage(BasePage):
	...
	def check_broken_link(self, url):
		...
		if request.status_code == 200:
			self.element_is_presents(self.locators.BAD_REQUEST).click()
		else:
...
class UploadAndDownloadPage(BasePage):
	...
	def update_file(self):
		...
		self.element_is_presents(self.locators.INPUT_UPLOAD).send_keys(path)
		os.remove(path)
		text = self.element_is_presents(self.locators.UPLOADED_RESULT).text
		return file_name.split('\\' and '/')[-1], text.split('\\')[-1]

	def download_file(self):
		link = self.element_is_presents(self.locators.DOWNLOAD_FILE).get_attribute("href")
		link_b = base64.b64decode(link)
		path_name_file = rf'C:\Users\d_onishchuk\PycharmProjects\pythonProject/testfile{random.randint(0, 999)}.jpg'
...
class DynamicPropertiesPage(BasePage):
	...
	def check_enable_button(self):
		try:
			self.element_is_clickable(self.locators.WILL_ENABLE_FIVE_SECONDS)
