...
class SelectablePage(BasePage):
    ...
    def select_list_item(self):
        ...
        self.click_selectable_item(self.locators.NOACTIVE_LIST_BUTTONS)
        active_element = self.element_is_visible(self.locators.ACTIVE_LIST_BUTTONS)
        return active_element.text

    def select_grid_item(self):
        self.element_is_visible(self.locators.GRID_TAB).click()
        self.click_selectable_item(self.locators.NOACTIVE_GRID_BUTTONS)
        active_element = self.element_is_visible(self.locators.ACTIVE_GRID_BUTTONS)
        return active_element.text
...
class ResizablePage(BasePage):
    ...
    def get_max_min_size(self, element):
        size = self.element_is_presents(element)
        size_value = size.get_attribute('style')
        return size_value

    def change_size_resizable_box(self):
        self.scroll_page(100)
        self.action_drag_and_drop_by_offset(self.element_is_presents(self.locators.RESIZABLE_BOX_HANDLE), 400, 200)
        max_size = self.get_px_from_width_height(self.get_max_min_size(self.locators.RESIZABLE_BOX))
        self.action_drag_and_drop_by_offset(self.element_is_presents(self.locators.RESIZABLE_BOX_HANDLE), -500, -300)
